...
void hidInit(void)
{
  ...
  gecko_cmd_hardware_set_soft_timer(TIMER_STOP, HID_TIMER, false);  /* HID Timer is initially inactive */
...
#ifdef HOST
    gecko_cmd_hardware_set_soft_timer(TIMER_MS_2_TIMERTICK(HID_IND_TIMEOUT), HID_TIMER, true);
#endif /* HOST */
  } else {
	/* indications switched off */
    gecko_cmd_hardware_set_soft_timer(TIMER_STOP, HID_TIMER, false);
...
void hidSendKeyboardText(void)
{
  ...
  /* Send character only if CCC is enabled and we are connected and bonded */
